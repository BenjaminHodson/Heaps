...
class BinomialHeap
{
    ...
    private int size;
    static HashMap<String, BinomialHeap> BinHeapList = new HashMap<>();
    String name;
 
    /* Constructor */
    public BinomialHeap(String name)
    {
        Nodes = null;
        size = 0;
        this.name = name;
        BinHeapList.put(this.name, this);
    }
    ...
    {
        BinomialHeapNode temp = Nodes.findNodeWithKey(old_value);
        if (temp == null)
            return;
        temp.key = new_value;
    ...
    private void printHeap(BinomialHeapNode r)     
    {
        if (r != null)
        {
            printHeap(r.child);
            Main.writer.print(r.key +", ");
            printHeap(r.sibling);
